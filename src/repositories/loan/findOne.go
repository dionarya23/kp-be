...
func (i *sLoanRepository) FindOne(loanId int64, customerId int64) (*entities.Loan, error) {
	...
	query.WriteString("SELECT id, customer_id, amount, tenor, purpose, interest_rate, monthly_installment, status, created_at, updated_at FROM Loans WHERE id = ? AND customer_id = ?")
